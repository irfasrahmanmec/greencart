...
const ProductCard = ({ product }) => {
    ...
    return product && (
        <div className="grid grid border border-gray-500/20 rounded-md md:px-4 px-3 py-2 bg-red-500 min-w-56 max-w-56 w-full  ">
                ...
                <div className="flex items-end justify-between mt-3">
                    ...
                    <div onClick={(e) => {e.stopPropagation(); }} className="text-primary">
                        {!cartItems[product._id] ? (
                            <button
                                className="flex items-center justify-center gap-1 bg-primary/10 border border-primary/40  md:w-[80px] w-[64px] h-[34px] 
                                rounded cursor-pointer "
                                onClick={() => addToCart(product._id)}
                            >
                                <img className="md:w-4 w-3" src={assets.cart_icon} alt="cart_icon" />
                                Add
                            </button>
                        ) : (
